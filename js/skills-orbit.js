...
import {
  CSS2DRenderer,
  CSS2DObject,
} from 'https://cdn.jsdelivr.net/npm/three@0.157.0/examples/jsm/renderers/CSS2DRenderer.js';
...
window.addEventListener('load', () => {
  ...
  skillsData.forEach((skill, index) => {
    ...
    const skillIndexInOrbit = skillsData
      .slice(0, index)
      .filter((s) => s.orbit === orbitIndex).length;
  ...
  function animate() {
    ...
    skills.forEach((skill) => {
      ...
      const position = new THREE.Vector3(Math.cos(angle) * 3, 0, Math.sin(angle) * 3);
      ...
      position.applyAxisAngle(
        ...
        skill.orbit.rotation.y - skill.orbit.userData.initialRotationY
      );
